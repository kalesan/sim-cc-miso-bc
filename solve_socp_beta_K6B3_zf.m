function m = solve_socp_beta_K6B3_zf(h, m_p, y_p, SNR, N0, S, beta)
    [L, K] = size(h);
    ...
    p_p = zeros(msgs, groups, groupings);
    for n = 1:msgs for g = 1:groupings, for f = 1:groups
        p_p(n, f, g) = norm(m_p(:, n, f, g));
    end, end, end

    m_zf = zeros(size(m_p));

    for g = 1:groupings, 
        for msg = 1:msgs for f = 1:groups
            tmp = null(h(:, setdiff(1:K, G(MAI(msg, :), g, f))')');
            [U, D, V] = svd(h'  * tmp);

            m_zf(:, msg, f, g) = tmp  * V(:, 1);
            m_z(:, msg, f, g) = m_zf(:, msg, f, g) / norm(m_zf(:, msg, f, g));

            m_p(:, msg, f, g) = m_zf(:, msg, f, g) * p_p(msg, f, g);
        end, end
    end
    ...
    cvx_begin
        ...
        variable p(msgs, groups, groupings);
        ...
        subject to
            ...
            % ZF beamformers
            for msg = 1:msgs, for f = 1:groups, for g = 1:groupings
                m(:, msg, f, g) == m_zf(:, msg, f,g ) * p(msg, f, g);
            end, end, end
